...
@DevEnv Claims\Claims.sln /Build Debug /Out Build.log

@cd FactRetrieverForClaimsProcessing
@IF NOT EXIST %AssemblyKeyFile2% sn -k %AssemblyKeyFile2%
@cd ..

@DevEnv FactRetrieverForClaimsProcessing\FactRetrieverForClaimsProcessing.sln /Build Debug /Out Build.log

@ECHO.
@ECHO Building the sample ...

@DevEnv RulesForMedicalClaims\%SolutionName% /Build Debug /Out Build.log
