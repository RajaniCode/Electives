...
namespace Microsoft.Samples.BizTalk.Adapters.TransportProxyUtils
{
	...
	internal class ResponseCallback : IBTTransmitter
	{
		...
		public ResponseCallback(TpBatchAsyncCallback batchCallBack)
		{
...

		{
			...
			response.AddPart(msg.BodyPartName, msg.GetPart(msg.BodyPartName), true);                                 

			// Replace the body part stream with the memory stream
			memStrm.Seek(0, SeekOrigin.Begin);
			response.BodyPart.Data = memStrm;
			_msg = response;
			...
			// Return true to inform the messaging engine we have ownership of the message
			return true;
		}
