...
namespace PayPal
{
    ...
    internal class Logger
    {
        ...
        private Logger(Type type)
        {
            ...
            ConfigureLoggers();
            PayPalConfiguration.PropertyChanged += ConfigsChanged;
        }
        ...
        {
            LoggerTypes logging = PayPalConfiguration.Logging;
            foreach (InternalLogger il in loggers)
            {
                if (il is InternalLog4netLogger)
                    il.IsEnabled = (logging & LoggerTypes.LOG4NET) == LoggerTypes.LOG4NET;
                if (il is InternalSystemDiagnosticsLogger)
                    il.IsEnabled = (logging & LoggerTypes.DIAGNOSTICS) == LoggerTypes.DIAGNOSTICS;
            }
        }
    ...
    internal class InternalLog4netLogger : InternalLogger
    {
        ...
        private static void loadStatics()
        {
            lock (InternalLog4netLogger.LOCK)
            {
                ...
                try
                {
                    loggerType = Type.GetType("PayPal.Logger");
                    ...
                    if ((PayPalConfiguration.Logging & LoggerTypes.LOG4NET) == LoggerTypes.LOG4NET)
                    {
    ...
    /// Example: if type is PayPal.DynamoDB.AmazonDynamoDBClient, listeners can be configured for:
    /// -PayPal.DynamoDB.AmazonDynamoDBClient
    /// -PayPal.DynamoDB
    /// -PayPal
