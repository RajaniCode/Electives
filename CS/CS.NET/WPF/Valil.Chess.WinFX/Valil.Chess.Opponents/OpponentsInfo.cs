...
namespace Valil.Chess.Opponents
{
    ...
    public class OpponentsInfo : Component, INotifyPropertyChanged
    {
        ...
        {
            // set the opponents
            if (OpponentsSettings.Default.LocalEngineOpponent)
            {
                opponents = new HumanVsLocalEngine();
            }
            else if (OpponentsSettings.Default.TwoPlayersMode)
            {
        ...
        private void property_Changed(object sender, PropertyChangedEventArgs e)
        {
            ...
            if (e.PropertyName == "LocalEngineOpponent" && OpponentsSettings.Default.LocalEngineOpponent && !(opponents is HumanVsLocalEngine))
            {
            ...
            else if (e.PropertyName == "TwoPlayersMode" && OpponentsSettings.Default.TwoPlayersMode && !(opponents is NoAIOpponent))
            {
                // dispose the old opponents
                opponents.Dispose();
        ...
        private void SetOpponentsInfoText()
        {
            if (OpponentsSettings.Default.LocalEngineOpponent)
            {
                OpponentsInfoText = OpponentsSettings.Default.AIOpponentPlaysWhite ?
                    Resources.ComputerVsYouText :
                    Resources.YouVsComputerText;
            }
            else if (OpponentsSettings.Default.TwoPlayersMode)
            {
