...
Sys.UI._DragDropManager =function(){var _instance;this.dragStart =new Type.Event(null,false);this.dragStop =new Type.Event(null,false);this._getInstance =function(){if (_instance ==null){if (Sys.Runtime.get_hostType()==Sys.HostType.InternetExplorer){_instance =new Sys.UI.IEDragDropManager();}
else {_instance =new Sys.UI.GenericDragDropManager();}
_instance.initialize();_instance.dragStart.add(Function.createDelegate(this,this._raiseDragStart));_instance.dragStop.add(Function.createDelegate(this,this._raiseDragStop));}
return _instance;}
...
this._prepareForDomChanges();dragSource.onDragStart();var eventArgs =new Sys.UI.DragDropEventArgs(dragSource.get_dragMode(),dragSource.get_dataType(),dragSource.get_data());this.dragStart.invoke(this,eventArgs);this._recoverFromDomChanges();this._wireEvents();this._drag(true);}
Sys.UI.IEDragDropManager.registerBaseMethod(this,'startDragDrop');this._stopDragDrop =function(cancelled){var ev =window.event;if (_activeDragSource !=null){this._unwireEvents();if (!cancelled){cancelled =(_underlyingTarget ==null);}
...
_activeDragSource.onDragEnd(cancelled);this.dragStop.invoke(this,Sys.EventArgs.Empty);_activeDragVisual.style.position =_activeDragVisual.originalPosition;_activeDragSource =null;_activeContext =null;_activeDragVisual =null;_isDragging =false;_potentialTarget =null;ev.returnValue =false;}
}
Sys.UI.IEDragDropManager.registerBaseMethod(this,'_stopDragDrop');this._drag =function(isInitialDrag){var ev =window.event;var mousePosition ={x:ev.clientX,y:ev.clientY };document._lastPosition =mousePosition;var scrollOffset =this.getScrollOffset(_activeDragVisual,true);var position =this.addPoints(this.subtractPoints(mousePosition,_activeDragVisual.startingPoint),scrollOffset);if (!isInitialDrag &&parseInt(_activeDragVisual.style.left)==position.x &&parseInt(_activeDragVisual.style.top)==position.y){return;}
Sys.UI.Control.setLocation(_activeDragVisual,position);this._prepareForDomChanges();_activeDragSource.onDrag();this._recoverFromDomChanges();_potentialTarget =this._findPotentialTarget(_activeDragSource,_activeDragVisual);var movedToOtherTarget =(_potentialTarget !=_underlyingTarget ||_potentialTarget ==null);if (movedToOtherTarget &&_underlyingTarget !=null){this._leaveTarget(_activeDragSource,_underlyingTarget);}
...
Sys.UI.IEDragDropManager.registerBaseMethod(this,'_unwireEvents');this.registerDropTarget =function(dropTarget){if (_dropTargets ==null){_dropTargets =[];}
_dropTargets.add(dropTarget);this._wireDropTargetEvents(dropTarget);}
Sys.UI.IEDragDropManager.registerBaseMethod(this,'registerDropTarget');this.unregisterDropTarget =function(dropTarget){this._unwireDropTargetEvents(dropTarget);_dropTargets.remove(dropTarget);}
Sys.UI.IEDragDropManager.registerBaseMethod(this,'unregisterDropTarget');this._wireDropTargetEvents =function(dropTarget){var associatedElement =dropTarget.get_dropTargetElement();associatedElement._dropTarget =dropTarget;associatedElement.attachEvent("ondragenter",_dragEnterHandler);associatedElement.attachEvent("ondragleave",_dragLeaveHandler);associatedElement.attachEvent("ondragover",_dragOverHandler);associatedElement.attachEvent("ondrop",_dropHandler);}
...
else {var dataObjects =Sys.UI.IEDragDropManager._getDataObjectsForDropTarget(this._getDropTarget(ev.srcElement));for (var i =0;i <dataObjects.length;i++){this._dropTarget.onDragEnterTarget(Sys.UI.DragMode.Copy,dataObjects[i].type,dataObjects[i].value);}
}
}
function dragLeaveHandler(){var ev =window.event;if (_isDragging){ev.returnValue =false;}
else {var dataObjects =Sys.UI.IEDragDropManager._getDataObjectsForDropTarget(this._getDropTarget(ev.srcElement));for (var i =0;i <dataObjects.length;i++){this._dropTarget.onDragLeaveTarget(Sys.UI.DragMode.Copy,dataObjects[i].type,dataObjects[i].value);}
}
}
function dragOverHandler(){var ev =window.event;if (_isDragging){ev.returnValue =false;}
else {var dataObjects =Sys.UI.IEDragDropManager._getDataObjectsForDropTarget(this._getDropTarget(ev.srcElement));for (var i =0;i <dataObjects.length;i++){this._dropTarget.onDragInTarget(Sys.UI.DragMode.Copy,dataObjects[i].type,dataObjects[i].value);}
}
}
function dropHandler(){var ev =window.event;if (!_isDragging){var dataObjects =Sys.UI.IEDragDropManager._getDataObjectsForDropTarget(this._getDropTarget(ev.srcElement));for (var i =0;i <dataObjects.length;i++){this._dropTarget.drop(Sys.UI.DragMode.Copy,dataObjects[i].type,dataObjects[i].value);}
}
...
var type =dragSource.get_dataType();var mode =dragSource.get_dragMode();var data =dragSource.get_data(_activeContext);var scrollOffset =this.getScrollOffset(document.body,true);var x =ev.clientX +scrollOffset.x;var y =ev.clientY +scrollOffset.y;var cursorRect ={x:x -_radius,y:y -_radius,width:_radius *2,height:_radius *2 };var targetRect;for (var i =0;i <_dropTargets.length;i++){targetRect =Sys.UI.Control.getBounds(_dropTargets[i].get_dropTargetElement());if (Sys.UI.Control.overlaps(cursorRect,targetRect)&&_dropTargets[i].canDrop(mode,type,data)){return _dropTargets[i];}
}
return null;}
this._prepareForDomChanges =function(){_oldOffset =Sys.UI.Control.getLocation(_activeDragVisual);}
this._recoverFromDomChanges =function(){var newOffset =Sys.UI.Control.getLocation(_activeDragVisual);if (_oldOffset.x !=newOffset.x ||_oldOffset.y !=newOffset.y){_activeDragVisual.startingPoint =this.subtractPoints(_activeDragVisual.startingPoint,this.subtractPoints(_oldOffset,newOffset));scrollOffset =this.getScrollOffset(_activeDragVisual,true);var position =this.addPoints(this.subtractPoints(document._lastPosition,_activeDragVisual.startingPoint),scrollOffset);Sys.UI.Control.setLocation(_activeDragVisual,position);}
}
...
Sys.UI.IEDragDropManager.registerClass('Sys.UI.IEDragDropManager',Sys.Component);Sys.UI.IEDragDropManager._getDataObjectsForDropTarget =function(dropTarget){if (dropTarget ==null){return [];}
var ev =window.event;var dataObjects =[];var dataTypes =["URL","Text"];var data;for (var i =0;i <dataTypes.length;i++){data =ev.dataTransfer.getData(dataTypes[i]);if (dropTarget.canDrop(Sys.UI.DragMode.Copy,dataTypes[i],data)){if (data !=null){dataObjects.add({type :dataTypes[i],value :data });}
}
}
return dataObjects;}
Sys.UI.GenericDragDropManager =function(){Sys.UI.GenericDragDropManager.initializeBase(this);var _dropTargets;var _scrollEdgeConst =40;var _scrollByConst =10;var _scroller;var _scrollDeltaX;var _scrollDeltaY;var _activeDragVisual;var _activeContext;var _activeDragSource;var _oldOffset;var _potentialTarget;var _mouseUpHandler;var _mouseMoveHandler;var _keyPressHandler;this.initialize =function(){Sys.UI.GenericDragDropManager.callBaseMethod(this,"initialize");_mouseUpHandler =Function.createDelegate(this,mouseUpHandler);_mouseMoveHandler =Function.createDelegate(this,mouseMoveHandler);_keyPressHandler =Function.createDelegate(this,keyPressHandler);if (window.__safari){Sys.UI.GenericDragDropManager.__loadSafariCompatLayer(this);}
_scroller =new Sys.Timer();_scroller.set_interval(10);_scroller.tick.add(scrollerTickHandler);}
this.startDragDrop =function(dragSource,dragVisual,context){_activeDragSource =dragSource;_activeDragVisual =dragVisual;_activeContext =context;Sys.UI.GenericDragDropManager.callBaseMethod(this,"startDragDrop",[dragSource,dragVisual,context]);}
this._stopDragDrop =function(cancelled){_scroller.set_enabled(false);Sys.UI.GenericDragDropManager.callBaseMethod(this,"_stopDragDrop",[cancelled]);}
this._drag =function(isInitialDrag){Sys.UI.GenericDragDropManager.callBaseMethod(this,"_drag",[isInitialDrag]);this._autoScroll();}
...
function scrollerTickHandler(){var oldLeft =document.body.scrollLeft;var oldTop =document.body.scrollTop;window.scrollBy(_scrollDeltaX,_scrollDeltaY);var newLeft =document.body.scrollLeft;var newTop =document.body.scrollTop;var dragVisual =_activeDragVisual;var position ={x:parseInt(dragVisual.style.left)+(newLeft -oldLeft),y:parseInt(dragVisual.style.top)+(newTop -oldTop)};Sys.UI.Control.setLocation(dragVisual,position);}
}
Sys.UI.GenericDragDropManager.registerSealedClass('Sys.UI.GenericDragDropManager',Sys.UI.IEDragDropManager);if (window.__safari){Sys.UI.GenericDragDropManager.__loadSafariCompatLayer =function(ddm){ddm._getScrollOffset =ddm.getScrollOffset;ddm.getScrollOffset =function(element,recursive){return {x:0,y:0 };}
...
Type.createEnum('Sys.UI.RepeatDirection','Horizontal',0,'Vertical',1);Sys.UI.DragDropList =function(){Sys.UI.DragDropList.initializeBase(this);var _isDragging;var _acceptedDataTypes =[];var _dataType;var _dragMode;var _dragVisual;var _direction =Sys.UI.RepeatDirection.Vertical;var _emptyTemplate;var _emptyTemplateInstance;var _dropCueTemplate;var _dropCueTemplateInstance;var _floatContainerInstance;var _originalParent;var _originalNextSibling;var _originalZIndex;var _currentContext;var _data;this.get_data =function(){return _data;}
this.set_data =function(value){_data =value;}
this.initialize =function(){Sys.UI.DragDropList.callBaseMethod(this,'initialize');this.control.element.__dragDropList =this;Sys.UI.DragDropManager.registerDropTarget(this);}
this.getDescriptor =function(){var td =Sys.UI.DragDropList.callBaseMethod(this,'getDescriptor');td.addProperty("acceptedDataTypes",Array);td.addProperty("data",Object);td.addProperty("dataType",String);td.addProperty("emptyTemplate",Sys.UI.ITemplate);td.addProperty("dropCueTemplate",Sys.UI.ITemplate);td.addProperty("dropTargetElement",Object,true);td.addProperty("direction",Sys.UI.RepeatDirection);td.addProperty("dragMode",Sys.UI.DragMode);return td;}
Sys.UI.DragDropList.registerBaseMethod(this,'getDescriptor');this.startDragDrop =function(dragObject,context,dragVisual){if (!_isDragging){_isDragging =true;_currentContext =context;if (dragVisual ==null){dragVisual =this.createDragVisual(dragObject);}
else {_dragVisual =dragVisual;}
Sys.UI.DragDropManager.startDragDrop(this,dragVisual,context);}
}
this.createDragVisual =function(dragObject){if (_dragMode ==Sys.UI.DragMode.Copy){_dragVisual =dragObject.cloneNode(true);}
else {_dragVisual =dragObject;}
var oldOffset =Sys.UI.DragDropManager._getInstance().getScrollOffset(dragObject,true);_dragVisual.style.width =dragObject.offsetWidth +"px";_dragVisual.style.height =dragObject.offsetHeight +"px";_dragVisual.style.opacity ="0.4";_dragVisual.style.filter ="progid:DXImageTransform.Microsoft.BasicImage(opacity=0.4);";_originalZIndex =_dragVisual.style.zIndex;_dragVisual.style.zIndex =99999;_originalParent =_dragVisual.parentNode;_originalNextSibling =Sys.UI.DragDropManager._getInstance().getNextSibling(_dragVisual);var currentLocation =Sys.UI.Control.getLocation(dragObject,true);var dragVisualContainer =this._getFloatContainer();Sys.UI.Control.setLocation(dragVisualContainer,currentLocation);if (Sys.UI.DragDropManager._getInstance().hasParent(_dragVisual)){_dragVisual.parentNode.removeChild(_dragVisual);}
dragVisualContainer.appendChild(_dragVisual);var newOffset =Sys.UI.DragDropManager._getInstance().getScrollOffset(dragObject,true);if (oldOffset.x !=newOffset.x ||oldOffset.y !=newOffset.y){var diff =Sys.UI.DragDropManager._getInstance().subtractPoints(oldOffset,newOffset);Sys.UI.Control.setLocation(dragVisualContainer,Sys.UI.DragDropManager._getInstance().subtractPoints(currentLocation,diff));}
...
this.dispose =function(){this.control.element.__dragDropList =null;Sys.UI.DragDropList.callBaseMethod(this,'dispose');}
this.onDragStart =function(){this._validate();}
this.onDrag =function(){}
this.onDragEnd =function(cancelled){if (_floatContainerInstance !=null){if (_dragMode ==Sys.UI.DragMode.Copy){_floatContainerInstance.removeChild(_dragVisual);}
...
if (!cancelled &&_data !=null &&_dragMode ==Sys.UI.DragMode.Move){var data =this.get_data(_currentContext);if (_data !=null &&data !=null){_data.remove(data);}
}
...
this.canDrop =function(dragMode,dataType,data){for (var i =0;i <_acceptedDataTypes.length;i++){if (_acceptedDataTypes[i]==dataType){return true;}
}
return false;}
this.drop =function(dragMode,dataType,data){if (dataType =="HTML"&&dragMode ==Sys.UI.DragMode.Move){dragVisual =data;var potentialNextSibling =this._findPotentialNextSibling(dragVisual);this._setDropCueVisible(false,dragVisual);dragVisual.parentNode.removeChild(dragVisual);if (potentialNextSibling !=null){this.control.element.insertBefore(dragVisual,potentialNextSibling);}
else {this.control.element.appendChild(dragVisual);}
}
else {this._setDropCueVisible(false);}
if (_data !=null &&data !=null){var newRow =data;if (Sys.Data.DataRow.isInstanceOfType(data)&&Sys.Data.DataTable.isInstanceOfType(_data)){var src =data.get_table();if (src){newRow =_data.createRow(data);}
}
_data.add(newRow);}
}
...
var potentialNextSibling =this._findPotentialNextSibling(dragVisual);if (!Sys.UI.DragDropManager._getInstance().hasParent(_dropCueTemplateInstance)){if (potentialNextSibling !=null){this.control.element.insertBefore(_dropCueTemplateInstance,potentialNextSibling);}
...
else {if (Sys.UI.DragDropManager._getInstance().getNextSibling(_dropCueTemplateInstance)!=potentialNextSibling){this.control.element.removeChild(_dropCueTemplateInstance);if (potentialNextSibling !=null){this.control.element.insertBefore(_dropCueTemplateInstance,potentialNextSibling);}
else {this.control.element.appendChild(_dropCueTemplateInstance);}
}
}
}
else {if (_dropCueTemplateInstance !=null &&Sys.UI.DragDropManager._getInstance().hasParent(_dropCueTemplateInstance)){this.control.element.removeChild(_dropCueTemplateInstance);}
}
}
}
this._findPotentialNextSibling =function(dragVisual){var dragVisualRect =Sys.UI.Control.getBounds(dragVisual);var isVertical =(_direction ==Sys.UI.RepeatDirection.Vertical);var nodeRect;for (var node =this.control.element.firstChild;node !=null;node =node.nextSibling){if (node.innerHTML !=null &&node !=_dropCueTemplateInstance &&node !=_emptyTemplateInstance){nodeRect =Sys.UI.Control.getBounds(node);if ((!isVertical &&dragVisualRect.x <=nodeRect.x)||(isVertical &&dragVisualRect.y <=nodeRect.y)){return node;}
}
}
return null;}
...
else if (!Sys.UI.DragDropManager._getInstance().hasParent(_emptyTemplateInstance)){this.control.element.appendChild(_emptyTemplateInstance);}
}
else {if (_emptyTemplateInstance !=null &&Sys.UI.DragDropManager._getInstance().hasParent(_emptyTemplateInstance)){this.control.element.removeChild(_emptyTemplateInstance);}
}
}
}
...
Sys.UI.DragDropList.registerSealedClass('Sys.UI.DragDropList',Sys.UI.Behavior,Sys.UI.IDragSource,Sys.UI.IDropTarget,Sys.IDisposable);Sys.TypeDescriptor.addType('script','dragDropList',Sys.UI.DragDropList);Sys.UI.DataSourceDropTarget =function(){Sys.UI.DataSourceDropTarget.initializeBase(this);var _acceptedDataTypes;var _append =true;var _target;var _property ="data";this.get_append =function(){return _append;}
...
this.get_acceptedDataTypes =function(){return _acceptedDataTypes;}
this.set_acceptedDataTypes =function(value){_acceptedDataTypes =value;}
this.initialize =function(){Sys.UI.DataSourceDropTarget.callBaseMethod(this,'initialize');Sys.UI.DragDropManager.registerDropTarget(this);}
this.getDescriptor =function(){var td =Sys.UI.DataSourceDropTarget.callBaseMethod(this,'getDescriptor');td.addProperty("acceptedDataTypes",Array);td.addProperty("append",Boolean);td.addProperty("dropTargetElement",Object,true);td.addProperty("target",Object);td.addProperty("property",String);return td;}
this.get_dropTargetElement =function(){return this.control.element;}
this.canDrop =function(dragMode,dataType,data){for (var i =0;i <_acceptedDataTypes.length;i++){if (_acceptedDataTypes[i]==dataType){return true;}
}
return false;}
...
this.getDescriptor =function(){var td =Sys.UI.DraggableListItem.callBaseMethod(this,'getDescriptor');td.addProperty("data",Object);td.addProperty("handle",Object,false,Sys.Attributes.Element,true);td.addProperty("dragVisualTemplate",Sys.UI.ITemplate);return td;}
...
else if (!Sys.UI.DragDropManager._getInstance().hasParent(_dragVisualTemplateInstance)){this.control.element.appendChild(_dragVisualTemplateInstance);}
var location ={x:ev.clientX,y:ev.clientY };location =Sys.UI.DragDropManager._getInstance().addPoints(location,Sys.UI.DragDropManager._getInstance().getScrollOffset(document.body,true));Sys.UI.Control.setLocation(_dragVisualTemplateInstance,location);}
...
this.set_location =function(value){if (_location !=value){_location =value;if (this.get_isInitialized()){var numbers =_location.split(',');var location ={x :parseInt(numbers[0]),y :parseInt(numbers[1])};Sys.UI.Control.setLocation(this.control.element,location);}
this.raisePropertyChanged('location');}
}
this.initialize =function(){Sys.UI.FloatingBehavior.callBaseMethod(this,'initialize');Sys.UI.DragDropManager.registerDropTarget(this);var el =this.control.element;var location;if (_location){var numbers =_location.split(',');location ={x :parseInt(numbers[0]),y :parseInt(numbers[1])};}
else {location =Sys.UI.Control.getLocation(el);}
el.style.width =el.offsetWidth +"px";el.style.height =el.offsetHeight +"px";el.style.position ="absolute";Sys.UI.Control.setLocation(el,location);}
this.dispose =function(){Sys.UI.DragDropManager.unregisterDropTarget(this);if (_handle !=null){_handle.detachEvent("onmousedown",_mouseDownHandler);}
_mouseDownHandler =null;Sys.UI.FloatingBehavior.callBaseMethod(this,'dispose');}
...
if (undraggableTagNames.indexOf(tagName.toLowerCase())>-1){return false;}
return true;}
function mouseDownHandler(){var el =this.control.element;if (this.checkCanDrag(window.event.srcElement)){_dragStartLocation =Sys.UI.Control.getLocation(el);window.event.returnValue =false;this.startDragDrop(el);}
}
this.get_dataType =function(){return "_floatingObject";}
this.get_data =function(context){return null;}
this.get_dragMode =function(){return Sys.UI.DragMode.Move;}
this.onDragStart =Function.emptyMethod;this.onDrag =Function.emptyMethod;this.onDragEnd =function(canceled){if (!canceled){var cancelArgs =new Sys.CancelEventArgs();this.move.invoke(this,cancelArgs);canceled =cancelArgs.get_canceled();}
var el =this.control.element;if (canceled){Sys.UI.Control.setLocation(el,_dragStartLocation);}
else {var location =Sys.UI.Control.getLocation(el);_location =location.x +','+location.y;this.raisePropertyChanged('location');}
}
this.startDragDrop =function(dragVisual){Sys.UI.DragDropManager.startDragDrop(this,dragVisual,null);}
this.get_dropTargetElement =function(){return document.body;}
this.canDrop =function(dragMode,dataType,data){return (dataType =="_floatingObject");}
...
Sys.UI.FloatingBehavior.registerSealedClass('Sys.UI.FloatingBehavior',Sys.UI.Behavior,Sys.UI.IDragSource,Sys.UI.IDropTarget,Sys.IDisposable);Sys.TypeDescriptor.addType('script','floatingBehavior',Sys.UI.FloatingBehavior);