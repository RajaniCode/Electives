  ...
  if(mapView.myLocation != nil) {
    ...
    NSURLSessionDataTask *directionsTask = [self.markerSession dataTaskWithURL:directionsURL completionHandler:^(NSData *data, NSURLResponse *response, NSError *e) {
      ...
      if(!error) {
        ...
        /* TODO: On the main queue, first create a GMSPath using the encoded path from the JSON response.
         *       The encoded path can be accessed here: [@"routes"][0][@"overview_polyline"][@"points"]
         */
        

        
        /* TODO: Still on the main queue, use that GMSPath to create and display a GMSPolyline with the following parameters:
         *       width       4
         *       color       [UIColor colorWithRed:0.08627451 green:0.584313725 blue:0.929411765 alpha:1.0]
         */
...
// TODO: Remove the directionsLine from the map when a non-marker location is tapped
